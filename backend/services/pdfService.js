...
const generateCvHtml = (data) => {
    // Tam ismin farklı alanlardan (name, firstName, lastName) gelebilme olasılığına karşı kontrol
    const fullName = (data.personalInfo?.name || `${data.personalInfo?.firstName || ''} ${data.personalInfo?.lastName || ''}`.trim()).trim();

    // Veri alanlarının boş olup olmadığını kontrol eden ve boşsa o bölümü hiç göstermeyen yapı
    return `
    <html>
    <head>
        <meta charset="UTF-8">
        <style>
            ...
            .languages p, .certificates p, .projects p, .analysis p { 
                margin: 4px 0; 
            }
        </style>
    </head>
