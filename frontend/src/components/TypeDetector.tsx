...
type Target = { role?: string; seniority?: string; sector?: string; confidence?: number }
...
const SENIORITY = ['Intern','Junior','Mid','Senior','Lead','Manager','Director','VP','C-Level']
...
export default function TypeDetector({ cv, initialTarget, onChange }: Props) {
  ...
  useEffect(() => {
    let mounted = true
    if (!initialTarget || (!initialTarget.role && !initialTarget.seniority && !initialTarget.sector)) {
      setLoading(true)
      postTypeDetect({ cv })
        .then((res) => {
          if (!mounted) return
    ...
    <div className="w-full max-w-screen-md mx-auto px-4 py-3">
      <div className="bg-white/70 dark:bg-neutral-900/70 backdrop-blur rounded-lg p-4 shadow-sm border border-neutral-200 dark:border-neutral-800">
        <div className="flex items-center justify-between mb-3">
          ...
          {loading && <span className="text-xs text-neutral-500">Detecting...</span>}
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          {typeof target.confidence === 'number' && (
            <div className="text-xs text-neutral-500">Model confidence: {(target.confidence * 100).toFixed(0)}%</div>
          )}
        </div>
      </div>
      <div className="h-14" />
