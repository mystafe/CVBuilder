...
export default function SaveBar({ cv, target, extras }: Props) {
  ...
  const save = async () => {
    ...
    } catch (e: any) { setError(e.message || 'Save failed') } finally { setLoading('') }
  }
  ...
  const share = async () => {
    ...
    } catch (e: any) { setError(e.message || 'Share failed') } finally { setLoading('') }
  }
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 bg-white/95 dark:bg-neutral-900/95 border-t border-neutral-200 dark:border-neutral-800 px-4 py-3 flex items-center justify-between gap-3 z-40">
      <div className="text-xs text-neutral-500">{error ? <span className="text-red-600">{error}</span> : (shareUrl ? <a className="text-blue-600 underline" href={shareUrl} target="_blank" rel="noreferrer">{shareUrl}</a> : (draftId ? `Draft: ${draftId}` : 'Unsaved draft'))}</div>
